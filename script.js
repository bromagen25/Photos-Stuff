...
function loadGallery(galleryId) {
    if (galleries[galleryId]) {
        ...
        galleryElement.innerHTML = `<p class="text-center text-xl mt-8">No images found for this gallery.</p>`;
    }
}
...
function renderGallery() {
    ...
    currentGallery.images.forEach((url, index) => {
        ...
        imgContainer.innerHTML = `
            <img src="${url}" alt="Gallery Image ${index + 1}" class="gallery-image w-full h-auto object-cover rounded-xl transform duration-300 group-hover:scale-110">
...
downloadButton.addEventListener('click', () => {
    // Open a new tab to download the full-size image
    ...
    if (galleryId) {
        ...
        if (galleryCardsContainer) {
            ...
            Object.keys(galleries).forEach(galleryName => {
                ...
                card.href = `gallery.html?gallery=${encodeURIComponent(galleryName)}`;
                ...
                card.innerHTML = `
                    <div class="text-center">
                        ...
                        <img src="${galleries[galleryName].icon}" alt="Gallery Icon" class="w-24 h-24 rounded-full object-cover border-4 border-red-500 mb-4 mx-auto">
                        <p class="text-gray-600">${galleries[galleryName].description}</p>
                    </div>
                `;
